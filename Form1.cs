...
namespace Compilador3._0
{
    public partial class Form1 : Form
    {
        ...
        public void VerificarLexema()
        {
            ...
            string[] reservado = { "TRUE", "FALSE", "Stop", "stop", "Also", "also", "INICIA", "FIN", "either", "Either", "end", "End", "Number", "number", "Text", "text", "Boolean", "boolean", "Maybe", "maybe", "During", "during", "get", "Get", "Clear" };
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    dataGridView1.Rows.Add(unir_cad + "", "Palabra reservada del lenguaje", contar_lineas, contar_columas);
                    validar_reservado = '1';
            ...
            {
                dataGridView1.Rows.Add(unir_cad + "", "Tipo de dato Entero", contar_lineas, contar_columas);
            }
            else if (Regex.IsMatch(unir_cad, exp_delimitador))
            {
                dataGridView1.Rows.Add(unir_cad + "", "Dato delimitador", contar_lineas, contar_columas);

            }
            else if (Regex.IsMatch(unir_cad, exp_operadores))
            {
                dataGridView1.Rows.Add(unir_cad + "", "Tipo de dato operador aritmetico", contar_lineas, contar_columas);
            }
            else if (Regex.IsMatch(unir_cad, asignacion))
            {
                dataGridView1.Rows.Add(unir_cad + "", "Tipo de asignacion", contar_lineas, contar_columas);
            }
            else if (Regex.IsMatch(unir_cad, exp_comparador))
            {
                dataGridView1.Rows.Add(unir_cad + "", "Tipo de comparacion", contar_lineas, contar_columas);
            }
            else if (Regex.IsMatch(unir_cad, variable))
            {
                dataGridView1.Rows.Add(unir_cad + "", "Tipo Variable", contar_lineas, contar_columas);
            }
        ...
        private void compile()
        {
            ...
            if (contador_error_compilar == 0)
            {
                ...
                using (StreamWriter writer = new StreamWriter("C:\\Users\\sergi\\Desktop\\test(2).txt", false))
        ...
        public void Sintactico()
        {
            ...
            for (int x = 0; x < dataGridView1.RowCount; x++)
            {
                if ((dataGridView1.Rows[x].Cells[1].Value.ToString()).Equals("comment"))
                {
            ...
            if (contador_comentarios_fila_menos > 0)
            {
                ...
                {
                    if ((dataGridView1.Rows[s].Cells[1].Value.ToString()).Equals("comment"))
                    {
            ...
            for (int recorridox = 0; recorridox < lexi_a_sint.Length; recorridox++)
            {
                ...
                if (Regex.IsMatch(lexi_a_sint[recorridox], "INICIA"))
                {
                    recorridox += 1;
            ...
            for (recorrido = 0; recorrido < lexi_a_sint.Length; recorrido++)
            {
                ...
                if (Regex.IsMatch(lexi_a_sint[recorrido], "INICIA"))
                {
                    recorrido += 1;
                    if (Regex.IsMatch(lexi_a_sint[recorrido], ";"))
                    {

                        dataGridView2.Rows.Add("CORRECTA", "INICIALIZACION --> INICIA ; <-- " + " FILA: " + num_linea[recorrido]);
                        recorrido += 1;
                        inicio_correcto = "1";
                    }
                    else
                ...
                if (inicio_correcto == "0")
                {
                    if (recorrido == 0)
                    {
                        dataGridView2.Rows.Add("ERROR", "SE ESPERABA INICIALIZACION: --> INICIA ; <-- " + "ANTES DE FILA: " + num_linea[recorrido]);
        ...
        public void estruc_var_cadena()
        {
            ...
            string existe = "no";
            if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "Text"))
            {
                recorrido_sum += 1;
                if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "^var[(0-9)?]$"))
                {
                    ...
                    if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "#"))
                    {
                        ...
                        if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "^\".*\"$"))
                        {
                            ...
                            if (Regex.IsMatch(lexi_a_sint[recorrido_sum], ";"))
                            {
                                ...
                                {
                                    variables[contador_variables] = lexi_a_sint[recorrido_sum - 3];
                                    tipo_variables[contador_variables] = "Text";
                                    contador_variables += 1;
        ...
        public void estruc_var_cadenax()
        {
                ...
                if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "^var[(0-9)?]$"))
                {
                    ...
                    if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "#"))
                    {
                        ...
                        if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "^\".*\"$"))
                        {
                            ...
                            {
                                dataGridView2.Rows.Add("ERROR", "DECLARACION --> Text <--DEBE IR ANTES DE --> proceso ; <-- " + "FILA: " + num_linea[recorrido]);
                            }
                            else
                            {
                                dataGridView2.Rows.Add("ERROR", "DECLARACION --> Text <--DEBE IR ANTES DE --> proceso ; <-- " + "FILA: " + num_linea[recorrido]); recorrido_sum -= 1;
                            }
            ...
            //finaliza reconocimento de declaracion de variable Text

        }//fin public void estruc_var_
        ...
        public void estruc_ver()
        {
            ...
            if (Regex.IsMatch(lexi_a_sint[recorrido_sum], "ver"))
            {
                ...
                {
                    dataGridView2.Rows.Add("ERROR", "SE ESPERABA UN Text" + " FILA: " + num_linea[recorrido] + " COLUMNA: " + num_columna[recorrido_sum]); recorrido_sum -= 1;
                }
